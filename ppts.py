...
for t in range(len(datanames)):
    for inst in range(distribution_instances):
        for var in range(5):
            ...
            ####Phase-1
            ##############
            #4.Probabilistic Distance and Task Satisfaction based Assignment Algo (PDTA)
            def pdta_algo():
                ...
                for a in range(n_agents):
                    ...
                    task_sat = [sum(agents[initial_solution[j]]) - tasks[j] for j in range(m_tasks)] #v(C_j) - t_j
                    z_j = np.sum(np.array(task_sat) >= 0, axis=1)   #no. of constraint satisfied for each task till now
                    zeta_j = features / (z_j + 1)        #calculate zeta value for each coalition
                    
                    prob_numerator = (inv_dist * zeta_j)             #value of numerator
            ...
            #print(var)
            ### 4. PDTA + DS_SCSGA 
            pdta_start = time.time()
            sol4, count4 = pdta_algo()
            pdta_ds_sol, pdta_ds_val, pdta_ds_count = ds_scsga(sol4, count4)
            pdta_end = time.time()
            pdta_ds_time = (pdta_end - pdta_start)
    ...
    result.to_csv(result_dir_path+datanames[t]+'_ppts.csv')
    result.to_csv(result_dir_path+'ppts.csv', mode='a')
    result=[]