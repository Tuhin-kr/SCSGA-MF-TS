...
for t in range(len(datanames)):
    for inst in range(distribution_instances):
        for var in range(5):
            ...
            ################# DS-SCSGA-MF
            ####Phase-1
            #1.Distance based Balanced Assignment Algo (DBA)
            def dba_algo():
                ...
                for a in range(n_agents):
                    ...
                    for t in range(m_tasks):
                          curr_size = len(initial_solution[sort_idx[t]]) #check the current size of C_j
                          if (curr_size  < max_size):
                              initial_solution[sort_idx[t]].append(a)  #if C_j is not full then put a_i
                              break
            ...
            ### 1. DBA + DS_SCSGA 
            dba_start=time.time()
            sol1, count1 = dba_algo()
            dba_ds_sol, dba_ds_val, dba_ds_count = ds_scsga(sol1, count1)
            dba_end=time.time()
            dba_ds_time = (dba_end - dba_start)
            #print(var)
    ...
    result.to_csv(result_dir_path+datanames[t]+'_bics.csv')
    result.to_csv(result_dir_path+'bics.csv', mode='a')
    result=[]
